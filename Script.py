# -*- coding: utf-8 -*-
"""Dimorfismo raiz 1molar

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1V9-VdOMHwYK1un4eyFyYUMOk56Kjq2XK
...
# @title
# ==============================
# Criar variáveis sinalizadoras
# ==============================

df['has_MV2'] = df[' MV2 (JAC)'].notna().astype(int)
df['has_P'] = df['P (JAC)'].notna().astype(int)

# ==============================
# Substituir NaN por 0
# ==============================

df[' MV2 (JAC)'] = df[' MV2 (JAC)'].fillna(0)
df['P (JAC)'] = df['P (JAC)'].fillna(0)

print("Variáveis sinalizadoras criadas com sucesso.")
print(df[[' MV2 (JAC)', 'has_MV2', 'P (JAC)', 'has_P']].head())
...
# @title
colunas = ['CÓD','sexo', 'max/mand', 'axial P (mesial)',
       'axial P (distal)', 'MV1 (JAC)', ' MV2 (JAC)', 'DV (JAC)', 'P (JAC)','has_MV2']

df = df.loc[:, colunas]

coluna_grupo = df.pop('sexo')
df.insert(0, 'sexo', coluna_grupo)
...
# pacientes únicos
codigos_unicos = df['CÓD'].drop_duplicates()

# split por paciente
cod_train, cod_test = train_test_split(
    codigos_unicos,
    test_size=0.2,
...
# separar linhas do dataframe conforme os pacientes sorteados
df_train = df[df['CÓD'].isin(cod_train)].copy()
df_test = df[df['CÓD'].isin(cod_test)].copy()

# voltar à sua estrutura original, sem CÓD como preditor
X_train = df_train.drop(columns=['CÓD', 'sexo'])
X_test = df_test.drop(columns=['CÓD', 'sexo'])
...
# @title
# Aesthetics upgrade (matplotlib only): horizontal CI "forest" style + sorted + clean labels
# - Easier to read than 30 rotated x labels
# - Same information: mean diff + 95% CI, with 0-reference line
# - No manual color specification (uses default cycle); sign encoded by marker shape + line style
...
# model_pairs = [...]
# y_test, y_probs, compare_auc_bootstrap already defined

# -----------------------------
# Compute mean diffs + CIs
# -----------------------------
mean_diffs, cis = [], []
for model1, model2 in model_pairs:
    mean_diff, ci_diff = compare_auc_bootstrap(
        np.array(y_test),
        np.array(y_probs[model1]),
        np.array(y_probs[model2])
    )
    mean_diffs.append(mean_diff)
    cis.append(ci_diff)

mean_diffs = np.array(mean_diffs, dtype=float)
lower_bounds = np.array([ci[0] for ci in cis], dtype=float)
upper_bounds = np.array([ci[1] for ci in cis], dtype=float)

labels = [f"{a.upper()} vs {b.upper()}" for a, b in model_pairs]

# -----------------------------
# Sort by effect size (cleaner story)
# -----------------------------
order = np.argsort(mean_diffs)  # ascending
mean_diffs = mean_diffs[order]
lower_bounds = lower_bounds[order]
upper_bounds = upper_bounds[order]
labels = [labels[i] for i in order]

# -----------------------------
# Plot: forest-style (recommended)
# -----------------------------
...
ax.hlines(ypos, lower_bounds, upper_bounds, linewidth=2, alpha=0.9)

# Point estimates: marker encodes sign
pos = mean_diffs >= 0
neg = ~pos
ax.scatter(mean_diffs[pos], ypos[pos], s=55, marker="o", alpha=0.95, label="Positive difference")
ax.scatter(mean_diffs[neg], ypos[neg], s=55, marker="^", alpha=0.95, label="Negative difference")
...
ax.set_xlabel("Difference in AUC (Model 1 − Model 2)", fontsize=12)
ax.set_title("Pairwise Differences in AUC with 95% Confidence Intervals", fontsize=13)

# Subtle grid
ax.grid(axis="x", linestyle="--", alpha=0.25)

# Make room and invert y so largest at top (optional)
ax.invert_yaxis()

# Legend (simple, clean)
ax.legend(loc="lower right", frameon=True)

# Optional: add small caption with abbreviations (cleaner than a big text box)
abbr = {
    "GB": "Gradient Boosting",
    "KNN": "K-Nearest Neighbors",
    "ADA": "AdaBoost",
...
caption = "Abbreviations: " + "; ".join([f"{k} = {v}" for k, v in abbr.items()])
fig.text(0.01, 0.01, caption, ha="left", va="bottom", fontsize=10)

plt.tight_layout(rect=(0, 0.03, 1, 1))
plt.savefig("Meandifference_forest.jpg", dpi=300, bbox_inches="tight")
plt.show()


# -----------------------------
# Alternative (if you insist on bars): cleaner vertical bar chart
# - sorted, thinner bars, CI caps, no legend box in plot area
# -----------------------------
fig, ax = plt.subplots(figsize=(14, 7))

x = np.arange(len(labels))
yerr = np.vstack([np.abs(mean_diffs - lower_bounds), np.abs(upper_bounds - mean_diffs)])

bars = ax.bar(x, mean_diffs, yerr=yerr, capsize=5, linewidth=0.8, alpha=0.9)

ax.axhline(0, linestyle="--", linewidth=1.8)
ax.set_xticks(x)
ax.set_xticklabels(labels, rotation=35, ha="right", fontsize=10)
ax.set_ylabel("Difference in AUC (Model 1 − Model 2)", fontsize=12)
ax.set_title("Pairwise Differences in AUC with 95% Confidence Intervals", fontsize=13)
ax.grid(axis="y", linestyle="--", alpha=0.25)

# Sign encoding by edge style (no explicit colors)
for b, md in zip(bars, mean_diffs):
    if md < 0:
        b.set_hatch("//")  # negative differences get hatch

fig.text(0.01, 0.01, caption, ha="left", va="bottom", fontsize=10)
plt.tight_layout(rect=(0, 0.03, 1, 1))
plt.savefig("Meandifference_bars_clean.jpg", dpi=300, bbox_inches="tight")
plt.show()